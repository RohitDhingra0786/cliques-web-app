...
const useMessages = () => {
  ...
  const { inboxDetails, allGroupsList } = useSelector((state) => state.message);
  ...
  useEffect(() => {
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState == "hidden") {
        handleOnlineOfflineStatus("false");
      } else {
    ...
    for (const obj of list) {
      ...
      if (obj.type == "group") {
        ...
        Object.keys(userData?.members || {}).forEach(function (key, index) {
          ...
          // )[0];
          // const groupuser = await
          let groupuser;
          getSingleUser("u_" + userValue?.user_id).then((res) => {
            groupuser = res;
          });
